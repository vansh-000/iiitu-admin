...
import TableForms from '../components/Tables/TableForms.jsx';

const TYPE = ['Student Form', 'Staff Form'];
...
const Forms = () => {
  ...
  const handleUpdate = async (id, updatedFormData) => {
    try {
      const response = await axios.put(`${API}/forms/${id}`, updatedFormData, {
        headers: {
      ...
      if (response.status === 200) {
        setData((prevData) =>
          prevData.map((item) =>
            item._id === id ? { ...item, ...updatedFormData } : item,
          ),
        );
